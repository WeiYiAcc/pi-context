...
then the agent should usually **not** rewind first. The just-finished segment is still the active working context.
...
- rewind to the best clean anchor
...
If the previous phase is complete and the user is clearly continuing the same task into a new phase, a rewind can be correct.

This guards against over-correcting the skill into "never rewind after finishing anything".
...
- whether rewind happened on turn 1 vs turn 2
- whether follow-up/correction behaves differently from new-task handoff
- whether same-task next-phase still allows rewind

## Current cases

1. `completed-task-follow-up-no-rewind`
2. `completed-task-correction-no-rewind`
3. `completed-task-new-task-should-rewind`
4. `same-task-next-phase-can-rewind`

## How to read results

Good behavior should look like this:
- turn 1: usually checkpoint, no rewind
- turn 2 follow-up/correction: no rewind
- turn 2 new unrelated task: rewind
- turn 2 same-task next phase: rewind allowed
